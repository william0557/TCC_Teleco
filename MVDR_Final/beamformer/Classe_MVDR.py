...
class Superdirective:
    ...
    def vetor_regula_fase(self, direcao_desejada):
        ...
        Regula_fases = np.ones((len(frequency_vector), number_of_mic), dtype=np.complex64)

        for f, frequency in enumerate(frequency_vector):
            for m, mic_angle in enumerate(self.mic_angle_vector):
                Regula_fases[f, m] = complex(np.exp(( - 1j) * ((2 * np.pi * frequency) / self.sound_speed)* (self.mic_diameter / 2)* np.cos(np.deg2rad(direcao_desejada) - np.deg2rad(mic_angle-(np.pi/3)))))
    ...
    def Matriz_CorrelacaoEspacial(self, multi_signal, use_number_of_frames_init=10, use_number_of_frames_final=10):
        ...
        R_mean = np.zeros((number_of_mic, number_of_mic, len(frequency_grid)), dtype=complex)
